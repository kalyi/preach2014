...
void Reach::MakeStatic() {
  g->fwd_edges = new int[g->ch_size_fwd];
  g->bwd_edges = new int[g->ch_size_bwd];
  g->el_fwd = (EdgeArray_t *)malloc(g->num_nodes * sizeof(EdgeArray_t));
  g->el_bwd = (EdgeArray_t *)malloc(g->num_nodes * sizeof(EdgeArray_t));
  g->MakeEdgeList(E_ECHFWD, g->el_fwd, g->fwd_edges);
  g->MakeEdgeList(E_ECHBWD, g->el_bwd, g->bwd_edges);
}
...
inline void Reach::BfsBwd(const int vid) {
  count++;
...
inline void Reach::BfsFwd(const int vid) {
  count++;
...
Reach::Reach(Graph *graph) {
  ...
  //std::cout << "size fwd: " << g->ch_size_fwd << " size bwd: " << g->ch_size_bwd << std::endl;
